...
Sixth (closes the legacy-install gap): `spawnSessionForTopic` now calls `ensureFrameworkIdentityFile` before every spawn — a new idempotent helper in `IdentityRenderer` that ensures the framework's shadow file (AGENTS.md for codex-cli, CLAUDE.md for claude-code) exists at the project root. When `.instar/AGENT.md` exists, it renders from there; when only a legacy shadow exists (older installs that authored CLAUDE.md directly without AGENT.md), it bootstraps AGENT.md from the legacy shadow first, then renders. Idempotent — no-op when the target shadow already exists, so the per-spawn cost is one stat call. Closes the gap that required dropping AGENTS.md by hand for the live test. 6 new unit tests.
...
Provider-portability v1.0.0 spawn-path closure: every remaining hot-path Claude hardcode shipped. New `buildHeadlessLaunch(framework, options)` companion to the existing `buildInteractiveLaunch` — same shape, same exhaustiveness check, same envOverrides contract. `SessionManager.spawnSession` (the headless `-p` prompt path) now routes through it with a per-call `framework` override and reads its binary from `frameworkBinaryPaths[framework]`. `PipeSessionSpawner`'s three direct `claude -p` callsites are gone: `classifyIntent` and `summarizeThreadHistory` accept an `IntelligenceProvider` (fail-closed to interactive/unavailable when none is wired — safer than the old silent shell-exec failure); `spawn()` uses `buildHeadlessLaunch` + `$(< "$file")` substitution so the prompt stays off the command line (no leak via `ps`). Env scrub now covers both `ANTHROPIC_API_KEY` AND `OPENAI_API_KEY` (Spec 12 Rule 1a extended to the pipe spawn). server.ts wires the pipe spawner with the resolved framework + sharedIntelligence. 11 new pipe-portability tests + buildHeadlessLaunch test coverage; 74/74 unit tests across the touched files green.

Provider-portability v1.0.0 tier-vocabulary unification: cross-framework generic tiers (`fast`|`balanced`|`capable`) are now the canonical model vocabulary. New `resolveModelForFramework(framework, modelOrTier)` helper maps the generic tiers to each framework's preferred concrete model (`fast`→`haiku`/`gpt-5.2`, `balanced`→`sonnet`/`gpt-5.3-codex`, `capable`→`opus`/`gpt-5.4`). Legacy Claude tier names map cleanly to Codex equivalents so unported callsites don't crash; raw model ids pass through verbatim. The Codex builder's hardcoded `'gpt-5.3-codex'` is now a `SessionManagerConfig.frameworkDefaultModels['codex-cli']` lookup — operators can override per agent without code changes. `UpgradeNotifyManager.MODEL_CHAIN` switched from Claude-only `['haiku','sonnet']` to generic `['fast','balanced']`; the spawn callback widened to accept generic tiers. `/sessions/spawn` route accepts a `framework` field and a framework-aware model whitelist that rejects Claude-only names when `framework=codex-cli` (and vice versa). `StallTriageNurse` model normalization is now framework-aware: Codex agents get `gpt-5.3-codex` for the default `balanced` tier instead of `claude-sonnet-4-6`. 5 new tier-resolution tests + 197/197 unit tests across 8 touched files green.
