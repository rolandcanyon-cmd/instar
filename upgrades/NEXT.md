...
### fix(presence-proxy): race-guard now ack-aware; Tier 1 skips LLM when only an ack is observed

PR #128 fixed the event-driven cancel path so brief acks ("Got it, looking into this") no longer kill standby tier timers — but it left a second cancel path untouched. That second path is a safety-net "race guard" that re-reads the messages log right before Tier 2 or Tier 3 fires; it was still seeing the ack as a real response and silently cancelling progressive updates. The same `isBriefAck()` classifier now feeds both paths, so the in-memory and on-disk views of "did the agent respond?" agree. Additionally, at the 20-second Tier 1 firing point, when the only post-message terminal activity is the agent's own ack, the proxy now emits a fixed "on this — I'll check back at the 2-minute mark" message instead of asking the LLM to paraphrase the ack (which previously read as a generic "Echo acknowledged and is investigating"). One token call saved, one clearer message produced.

Live failure signal reproduced: `[PresenceProxy] Skipping Tier 2 for topic 8882 — agent already responded (race guard)` at 2026-05-13T15:55:05Z, exactly two minutes after a user message and an ack that should have left the timer running. With the fix, that log line will not be emitted for ack-only messages.

Side-effects review: `upgrades/side-effects/presence-proxy-race-guard-ack.md` — covers over-block (ack classifier bias is the documented "false-positive costs one extra standby message"), under-block (longer first-touch replies that look conversational still cancel timers by design), level-of-abstraction fit (single `isBriefAck` export feeds both cancel paths and the new `'ack'` role on `conversationHistory`), signal-vs-authority compliance (internal lifecycle, not user-facing block/allow — closer to transport-layer dedup than judgment-gating), interactions (the two cancel paths now share semantics; no shadowing of downstream checks), external surfaces (Tier 1 placeholder message text only — no API contract changes; `lastAckText` is in-memory only, not persisted), and rollback (single revert; no schema or state-file changes).
...
Standby mode is fixed again. The earlier fix in version 0.28.84 only closed half the door: brief acks like "Got it, looking into this" stopped cancelling the standby timers in the real-time path, but a second safety-net path was still reading the same ack from the message log a couple of minutes later and cancelling the timer anyway. So your agent would send the first 20-second standby update and then go silent — exactly the regression you noticed. This release teaches that second path to recognise acks too, so the 2-minute and 5-minute progressive updates fire reliably. The first 20-second update is also cleaner: when the only thing your agent has done so far is acknowledge your message, the proxy now just says "on this — I'll check back at the 2-minute mark" instead of asking the language model to paraphrase the ack (which read as a generic "Agent acknowledged and is investigating").

Phase 1a let instar READ the new markdown-based job format. Phase 1b lets it RUN those jobs with the tool allowlist enforced. This release adds the CHECK side: when a signed lock-file is present, every instar-default job's body and configuration block are fingerprinted and compared against the locked entry before the job is allowed to run. If something has tampered with the file on disk (a corrupted sync, a rogue local process, a misconfigured update), the job is held back with a clear explanation and a path forward — instar will not silently fire a tampered system job. The build-pipeline side that actually signs the lock-file lands in a follow-up release; until then, every instar-default job loads marked as untrusted-no-lockfile, and downstream code uses that marker to refuse trust elevation. No setup required; the new behavior takes effect on the next agent update.
