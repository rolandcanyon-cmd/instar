...
2. Two fragments (`docs-coverage-issue-body-truncation.md`, `migration-consumer-lockstep-orphaned-diff-base.md`) sat directly in `upgrades/`, where `tests/unit/upgrade-guide-check.test.ts` requires a semver filename. **They were MISPLACED, not misnamed.** The canonical location for a pending per-PR fragment is `upgrades/next/<slug>.md` — a subdirectory the test never scans (`fs.readdirSync(upgradesDir)` does not recurse), which is why descriptive slugs are correct there. `scripts/pre-push-gate.js` names the convention directly and calls the flat file "legacy": `upgrades/next/<slug>.md (or legacy upgrades/NEXT.md)`. The retained side-effects artifact for one of them already cited its intended path as `upgrades/next/migration-consumer-lockstep-orphaned-diff-base.md`, so the files had simply landed a directory too high. Both are moved to `upgrades/next/` with content byte-identical to `39f6a5b06` (verified by diff) and their slugs intact.

   **A first attempt consolidated them into `upgrades/NEXT.md` instead. That was wrong twice over** and has been withdrawn: it used the path the gate calls legacy, and it merged two independent per-PR fragments into one file, which is not how a per-fragment release flow consumes them. It also introduced a file upstream does not carry, creating needless divergence for the daily rebase.

Files touched as shipped: two fragments moved `upgrades/*.md` → `upgrades/next/*.md`, content unchanged. No CI wiring change ships (see §1).
...
- `npx vitest run tests/unit/upgrade-guide-check.test.ts` — 7556 passed after the move to `upgrades/next/` (was 2 failing assertions before). The count differs from the interim `NEXT.md` attempt (7560) because the parameterised suite generates per-file cases, and the fragments no longer sit in the scanned directory.
...
- Also failing on the fork and NOT caused by this change: `tests/integration/subscription-pool-routes.test.ts` > "serves bounded login history on the literal route without confusing it for an account id" — expects `count: 1` with one episode, receives `count: 0` and an empty `episodes` array. **Reproduces locally (1 failed | 7 passed), so it is deterministic, not a CI timing flake.** Absent from run `33520200655` (2026-09-01); present after the rebase onto `46ac68d44` (v1.3.1219), which is a `[skip ci]` release commit, so upstream CI never ran against that exact tree. The single upstream commit the fork lacks (#2001, agent-signature labelling) touches messaging and ASP surfaces only — not this route — so pulling it will NOT fix this. Left untouched: diagnosing it is upstream's, and guessing at a fix would be worse than reporting it accurately.
