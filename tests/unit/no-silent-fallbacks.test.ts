...
describe('No Silent Fallbacks', () => {
  ...
  it('no new silent fallbacks beyond tracked baseline', () => {
    ...
    // ═══════════════════════════════════════════════════════════
    const BASELINE = 80; // 10 CapabilityMapper + 5 TelegramAdapter + 1 JobScheduler + 8 TopicResumeMap + 3 WhatsAppAdapter + 4 server.ts + 3 HookEventReceiver + 2 InstructionsVerifier + 2 SubagentTracker + 3 WorktreeMonitor + 1 AutonomousEvolution + 1 ExecutionJournal + 1 JobReflector + 1 ResumeValidator + 4 ContextualEvaluator + 4 CoherenceGate + 3 CustomReviewerLoader + 1 PolicyEnforcementLayer + 1 RecipientResolver + 5 ContextSnapshotBuilder + 1 AutoDispatcher + 1 JobRunHistory + 6 new-sentinel-resume-paths + 1 routes.ts (paste error handler) + 1 TreeTriage (LLM node triage catch) + 3 SoulManager (IntegrityManager sign) + 1 ConvergenceChecker + 1 pre-existing drift (unidentified) + 2 UpdateChecker/server.ts drift + 1 setup.ts (ensureStableNodeSymlink best-effort catch)
