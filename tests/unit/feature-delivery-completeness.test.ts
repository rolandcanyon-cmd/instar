...
describe('Feature Delivery Completeness', () => {
  ...
  describe('CLAUDE.md section parity: templates.ts ↔ PostUpdateMigrator', () => {
    ...
    const featureSections = [
      ...
      'Codex quota is first-class in the pool:', // Codex rollout quota joins the live pool control plane; framework-safe placement/swap is surfaced to Claude, Codex, and Gemini agents.
      'Solo Codex load shedding is fail-safe:', // Codex rollout quota also drives the solo-agent brake; missing/invalid readings shed instead of spawning into a wall.
      ...
      'Evolution action auto-expiry:', // conservative queue retention: template + existing-agent migrator + Codex/Gemini shadow parity; dry-run by default, protected actions retained, replicated removals tombstoned.
      ...
      'Self-Action Backpressure Governor', // unified-self-action-backpressure §11/LA9-1: the governor awareness block — GET /self-action-governor (per-class modes/counters/deciding-layer reasons), the three proactive "why was my respawn held / swap queued / notify folded?" triggers, and the emergencyDisable valve with the CONVERSATIONAL flip as the operator's mass-incident path. Full parity: templates.ts generateClaudeMd + migrator migrateClaudeMd + migrateFrameworkShadowCapabilities markers[]. Ships OBSERVE-ONLY fleet-wide (FD1); enforce is the operator's later per-class flip.
      'Durable Conversation Identity',
      'owned-identities', // correction-derived-hardening: Rung-0 owned-identities registry — full parity: templates.ts Rung-0 line + registration trigger, migrator '### Owned-Identities Registry' appended section, shadow marker for Codex/Gemini agents (they provision identities too; without the marker the founding wrong-escalation recurs on non-Claude frameworks) // durable-conversation-identity §6.2(b)/§9: the GET /conversations* read surface — a negative topicId is a MINTED conversation id (a Slack channel/thread), resolvable at /conversations/:id; /conversations/health is the tripwire surface. Full parity: templates.ts generateClaudeMd (### heading) + migrator migrateClaudeMd (### heading) + migrateFrameworkShadowCapabilities markers[]. Recording is always-on foundation (kill-switchable via conversationIdentity.recording.enabled); only DELIVERY (followThrough) is dev-gated.
    ];
    ...
    const legacyMigratorSections = [
      ...
      'Ultracode one-shot spawn', // Claude-specific prompt-keyword spawn option; migrate + fresh template share ULTRACODE_SPAWN_CLAUDEMD_SECTION. Deliberately absent from Codex/Gemini shadows because those frameworks reject the option.
      ...
      'Duplicate-Session Prevention', // ownership-gated-spawn Increment 1 (⚗️ dev-gated dark, observe-only): the SpawnAdmission/reconciler/ladder/provenance awareness section, added to BOTH generateClaudeMd and migrateClaudeMd via the shared DUPLICATE_RECONCILER_CLAUDEMD_SECTION(port) (single source, no drift) — the templates.ts SOURCE carries the interpolation call, not the literal, so it's tracked here like Machine Load Assessment / Dynamic MCP; dark dev-gated pool behavior, so no framework-shadow marker (the Dynamic MCP precedent).
      'Stall-Coverage Matrix Gate', // framework-stall-coverage-matrix §3.4/§3.5 item 5: the apprenticeship stall-coverage gate awareness (refusal class, live-read config knob, matrix-unverifiable-no-source honesty line, PIN acceptance route) — added to BOTH generateClaudeMd and migrateClaudeMd via the shared APPRENTICESHIP_STALL_GATE_CLAUDEMD_SECTION(port) (templates.ts carries the interpolation call, not the literal — the Duplicate-Session Prevention precedent); an apprenticeship-program maintainer capability, not a universal user-invokable one, so no framework-shadow marker (the Dynamic MCP precedent).
      ...
      'Test-Runner Concurrency Bound', // test-runner-concurrency-bound: the spawn-cap's sibling — behavioral-awareness section for the host-wide vitest-suite cap (a SAFETY FLOOR; ships WATCH-ONLY/dry-run for a 14-day soak) + the GET /test-runner-limiter read surface + the POST /test-runner-limiter/prune recovery lever + the INSTAR_HOST_TEST_SEMAPHORE kill switch — added via migrator AND emitted by the template directly (so a fresh init self-matches, never double-patched). Same migrator-only classification as 'Fork-Bomb Spawn Cap' above; not a user-invokable capability needing template-shadow parity (the /test-runner-limiter capability is already in CapabilityIndex).
      ...
      'non-gating internal calls also get a bounded', // nongating-failure-swap: the migrateClaudeMd content-sniff marker for the corrective "Non-Gating Failure-Swap" subsection (extends the provider-fallback framing above — non-gating internal calls also get a bounded, herd-safe swap). The TEMPLATE (generateClaudeMd) adds a bullet under the existing "Per-Component Framework Routing" section (a fresh init self-matches), while the migrator APPENDS a corrective subsection on this new marker (migrateClaudeMd only appends, never edits in place) — behavioral-awareness like 'run off Claude by default' above, not a separate user-invokable capability needing template-shadow parity.
      'Benchmark-Divergence Detector', // benchmark-divergence-detector (Increment A, dark+dryRun, observe-only): behavioral-awareness section (real grades vs benchmark predictions; GET /benchmark-divergence read surface; every finding advisory) — both generateClaudeMd (fresh init self-matches) AND migrateClaudeMd append it on this content-sniff marker via the shared BENCHMARK_DIVERGENCE_CLAUDEMD_SECTION, same migrator-only classification as the 'Autonomous Liveness Reconciler' / cartographer sections above (not a universal user-invokable capability needing template-shadow parity).
      ...
      'Doorway/Model Knowledge Registry', // DOORWAY-MODEL-KNOWLEDGE-REGISTRY-SPEC (rollout inc3): the GET /doorways read surface + the doorway-scan job that keeps it current. The scan job ships DARK (job manifest enabled:false); the read route is always-mounted with the D5 two-state contract. Migrator-only awareness (added via migrateClaudeMd content-sniff; also emitted by the template so a fresh init self-matches), not a universal user-invokable capability needing Codex/Gemini template-shadow parity (same classification as the Playwright Profile Registry / cartographer sections above).
      'Routing Spend view', // routing-control-room-spend Increment A: the read-only GET /routing-spend/summary + /caps surfaces + the Spend dashboard tab. Dev-gated (live on a development agent, dark on the fleet; routes 503 when off). Migrator-only awareness (added via migrateClaudeMd content-sniff + also emitted by the template so a fresh init self-matches), not a universal user-invokable capability needing Codex/Gemini template-shadow parity (same classification as the Doorway/Model Knowledge Registry section above).
      'Routing Spend MONEY layer', // routing-control-room-spend Increment B: the PIN-gated caps/arming plan flow + the Bearer freeze + GET /routing-spend/caps/log. Ships DARK for EVERYONE (routingSpend.money.enabled — a DARK_GATE_EXCLUSIONS action-bearing case; routes 503 when off). Migrator-only awareness (added via migrateClaudeMd content-sniff), same classification as the 'Routing Spend view' Increment-A section above (not a universal user-invokable capability needing Codex/Gemini template-shadow parity — the money controls are operator-PIN surfaces, not agent capabilities).
      'LLM-Decision Quality Meter', // llm-decision-quality-meter §6: the observe-only decision-quality read surface — GET /decision-quality (evidence-strength-first aggregates, 503-when-dark honesty), POST /decision-quality/grade-pass (deterministic, zero-LLM), the dark hourly llm-decision-grading job (enabled:false), and the census-debt re-surfacing. Dev-gated dark (provenance.uniformSeam, omit-required). Added via migrateClaudeMd content-sniff on the shared DECISION_QUALITY_CLAUDEMD_SECTION const AND emitted by the template (generateClaudeMd) so a fresh init self-matches, never double-patched — same migrator-only classification as 'Routing Spend view' / 'Doorway/Model Knowledge Registry' (an observability read surface, not a universal user-invokable capability needing Codex/Gemini template-shadow parity).
      'Session Listing Hygiene', // session-listing hygiene (CMT-1936): behavioral-awareness section — GET /sessions defaults to ACTIVE sessions (?include=all for the registry), finished records bounded by sessions.retention, pool.duplicateTopics flags only GENUINE cross-machine duplicates. Added via migrateClaudeMd content-sniff on the shared SESSION_LISTING_HYGIENE_CLAUDEMD_SECTION const AND emitted by the template (generateClaudeMd) so a fresh init self-matches, never double-patched — same migrator-only classification as 'Doorway/Model Knowledge Registry' / 'Routing Spend view' above (a read-surface behavior change, not a universal user-invokable capability needing Codex/Gemini template-shadow parity).
      'Audits run to convergence', // audit-convergence-enforcement §4: behavioral default-route section — audit-shaped work runs as the converging loop (single-pass = incomplete); canonical report at docs/audits/<slug>.md carries a machine-earned stamp. Emitted by generateClaudeMd AND added via migrateClaudeMd content-sniff on the shared AUDIT_CONVERGENCE_CLAUDEMD_SECTION const — same migrator-only classification as 'Session Listing Hygiene' (a behavioral-awareness section, not a user-invokable capability needing Codex/Gemini template-shadow parity).
      ...
      'Machine-Coherence Guard',                            // machine-coherence-guard (roadmap 4.1, F4/P0-1): the "### Machine-Coherence Guard" awareness section (GET /pool/machine-coherence read surface + the "why did I get a machine-coherence alarm?" proactive trigger + logs/machine-coherence.jsonl transition log), emitted by BOTH templates.ts generateClaudeMd (### heading) AND migrateClaudeMd (### heading content-sniff). Ships dev-gated dark on the fleet (monitoring.machineCoherence.enabled), dry-run FIRST on dev; a single-machine agent is a strict no-op. A dark monitoring-guard READ surface + behavioral trigger (a server-side coherence sentinel), not a universal user-invokable capability the Codex/Gemini frameworks shadow — same dark, template+migrator, not-framework-shadowed class as 'Write Admission' / 'Multi-Machine Session Pool (active-active'.
      'External-Hog Zombie Auto-Kill Sentinel',             // external-hog-zombie-autokill-sentinel §7-§8: the "### External-Hog Zombie Auto-Kill Sentinel" awareness section (GET /external-hog read surface + the PIN-gated POST /external-hog/arm + Bearer POST /external-hog/disarm levers + the "why did an outside process get flagged/cleaned up?" proactive trigger), emitted by BOTH templates.ts generateClaudeMd (via the shared EXTERNAL_HOG_CLAUDEMD_SECTION constant — the literal heading text lives in PostUpdateMigrator.ts, like the Mesh Self-Healing / Write Admission constants) AND migrateClaudeMd (content-sniff). Ships dev-gated dark on the fleet + watch-only on dev (a kill needs the operator's PIN arm); a dark monitoring-guard READ surface + operator lever, not a universal user-invokable capability the Codex/Gemini frameworks shadow — same dark, template+migrator, not-framework-shadowed class as 'Machine-Coherence Guard' / 'Write Admission'.
      'Rope-notice audit rows',                             // calm-transient-episode-alerting M-P3: the "### Rope-notice audit rows (calm-alerting)" migrator-only addendum — where demoted informational rope notices, hub fallbacks, and per-rope dedupe events land (logs/sentinel-events.jsonl) + the delivery-true-demotion rule. Behavioral READ-surface awareness the agent READS to answer "why did a rope notice go quiet?", not a user-invokable route the Codex/Gemini frameworks shadow — same migrator-only, behavioral, not-framework-shadowed class as 'Sentinel Notifications' / 'Machine-Coherence Guard'. The machine-coherence NARRATION content-update (calm/silent semantics) rides the already-tracked 'Machine-Coherence Guard' section's re-sniff, like '/cutover-readiness/import-dryrun' for Cutover Readiness.
      'Verify Before Done (observe-only v1)',                // correction-class-review-and-verify-before-done: completion-discipline awareness added by the migrator. Observe-only v1 records claims and evidence without blocking completion; this is behavioral guidance, not a framework-shadowed user-invokable capability.
    ];
