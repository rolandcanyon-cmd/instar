...
|---|---|---|---|---|
| `server/routes.ts` — `checkOutboundMessage()` calling `MessagingToneGate.review()` | Whether an outbound message contains enumerated leak patterns (B1–B9 + B11 rule set) | LLM-backed | **Authority — OK** | Sole outbound authority. Receives structured signals from junk, dedup, and paraphrase detectors plus recent conversation. `ToneReviewResult.rule` constrained to B1..B9 + B11; citations outside the set fail-open with `invalidRule=true` (2026-04-15 rework). B10_PARAPHRASE_FLAGGED is reserved for observability — the gate may cite it in dashboard telemetry but never blocks on it alone. B11_STYLE_MISMATCH blocks messages that significantly violate the agent's configured `messagingStyle` (ELI10, formal, terse, etc.); does not apply when `messagingStyle` is unset. |
