...
> ## ⚠️ AMENDMENT A1 — Read-access pivot: direct-DB → HTTP read seam (2026-05-28)
>
> **Status:** post-approval amendment, constraint-driven (not a design re-litigation). Authorized by Justin 2026-05-28 (topic 12476, "I trust your recommendation with the call") after Echo grounded the change against the live the-portal repo.
>
> **What changed and why.** The converged spec assumes Instar reads Portal's canonical feedback data via a **direct database connection** (a read-only Postgres role): see §2.2 "the ported processor … connects to the same cloud DB", §2.5 Phase-3 "both … write the **same** canonical database", and Part-4 Q2's resolution. That assumption is **infeasible on the actual platform**: **Prisma Data Platform forbids `CREATE ROLE` / `GRANT`**, so Portal cannot mint Instar the read-only DB role this design needs. This was discovered *after* convergence, during implementation.
>
> **The resolution (the read path).** Portal exposes an authenticated, read-only HTTP seam — `GET /api/instar/read` — and Instar reads through it. This is **already built**: Portal commit `d65136b3b6` ("feat: add /api/instar/read endpoint for Echo Phase 1 (Postgres role blocker workaround)") + docs `e1b9c2ac91`; Instar-side client `src/feedback-factory/dryrun/HttpParitySource.ts` (PR #463, merged). The seam is arguably *cleaner* than direct DB: it is the same contract the later write/update APIs use, it carries no Prisma-admin dependency, and the abstraction levels line up (Portal's HTTP surface ↔ Instar's `ParitySource` interface). `ReadOnlyShadowStore` is unaffected — it remains the dry-run **no-write guard** (a complement to the read seam, not a competing read path).
>
> **Scope of this amendment (deliberately narrow).** It resolves the **read-for-parity** path only (Phase-1 recorded-corpus parity needs no live access at all; Phase-3 live dual-forward reads Portal's clusters via this HTTP seam). It does **NOT** unilaterally rewrite the **write-side** architecture. The §2.5 precondition "ONE shared canonical DB through cutover" and the Phase-3/Phase-4 *write* mechanics interact with the same role constraint (how does Instar's processor *write* during dual-forward / post-cutover if it cannot hold a role on Portal's DB?) and that reconciliation is **re-opened as Q2b for Dawn's domain review** — it is her MIRROR design and her platform. This amendment does not pre-empt it.
>
> **Migration-history note.** A prior Echo session mislabeled this endpoint as "confabulation / does not exist" (a false negative from a stale or wrong checkout) and a memory entry enshrined a "do not build" directive. That is corrected: the endpoint, docs, and commits are real and on `the-portal:main`. The only accurate part of that earlier note: the endpoint is **committed but not yet deployed** (`portal.bot-me.ai/api/instar/read` returns 404 as of this amendment) — deploy + read-token handoff are the live Phase-1 unblock items, tracked outside this spec.
...
- **Processor placement** — the ported processor runs as an **Instar job** (`supervision: tier1`) on a capable machine (NOT the always-on Mini — it's heavyweight, batch, and bursty). It connects to the same cloud DB. This is the hosting-migration boundary: front → Vercel, heavy processor → capable machine, lightweight always-on (lifeline/listener/Ghost/MoltBridge) → Mini. **[AMENDED A1: "connects to the same cloud DB" holds for the operated instance's OWN DB, but Instar's READ of Portal's canonical data during migration/dry-run goes through the HTTP `/api/instar/read` seam, not a direct DB role — Prisma Data Platform forbids minting that role. See Amendment A1.]**
...
2. ~~**Canonical DB location**~~ — **RESOLVED (convergence): ONE shared canonical DB through cutover** (stay on the existing Prisma project; any relocation is a separate later step). A split DB would split-brain clustering (§2.5 precondition). **[PARTIALLY SUPERSEDED by Amendment A1.]**
2b. **READ-access mechanism + write-side under the Prisma-role constraint (re-opened by Amendment A1, 2026-05-28).** Prisma Data Platform forbids `CREATE ROLE`/`GRANT`, so Instar cannot hold a direct role on Portal's shared DB. **READ is resolved**: HTTP `/api/instar/read` seam (built — Portal `d65136b3b6`, Instar `HttpParitySource` #463). **Open for Dawn's domain review**: how Instar's processor *writes* to the shared canonical DB during Phase-3 dual-forward and post-Phase-4 cutover under the same role constraint — direct write is equally blocked, so the operated-instance write path needs the same kind of seam, or the operated instance owns its own DB and the "one shared DB through cutover" precondition is re-expressed in terms of the HTTP mirror rather than a shared connection. Her call (her MIRROR design, her platform).
