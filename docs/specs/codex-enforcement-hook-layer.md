...
**Project:** `codex-harness-followups` Tier 2 (item `codex-harness-followups-2`) <!-- tracked: codex-enforcement-hook-layer -->
...
| deferral / scope checkpoint | Stop | `Stop` | signal (no hard block) <!-- tracked: codex-enforcement-hook-layer --> |
...
- [~] **P4 — PermissionRequest trust integration (autonomy-preserving).** Route PermissionRequest → trust system, auto-decide, **zero human prompts**. Verify whether it fires under bypass mode; defensive no-op register if not. Unit + integration.
  - **STATE (2026-05-25):** `installCodexHooks` already wires `PermissionRequest → external-operation-gate.js`, which POSTs to `/operations/evaluate` (the trust authority) and auto-decides with no human prompt — the autonomy-preserving routing the phase asks for, already in place + unit-asserted (matcher `.*`, gate present on PermissionRequest). The OPEN part is the live-behavior question the spec itself flagged: under `--dangerously-bypass-approvals-and-sandbox` (the autonomous launch mode), Codex grants the operation without an approval round, so `PermissionRequest` most likely **never fires** — making it the defensive registration the spec anticipated ("if bypass suppresses it … PermissionRequest becomes a no-op we still register defensively"), with `PreToolUse` (wired + P5c-proven-blocking) as the real gate. Confirming fire-vs-suppress needs an interactive codex 0.133 run that surfaces a permission escalation. <!-- tracked: codex-enforcement-hook-layer --> Until then, the registration is correct and harmless either way; no code change required for autonomy-preservation.
...
- [x] **P6a — ✅ DONE: autonomy via the launch flag (per-agent), NOT system-managed hooks.** Trust is a SEPARATE concern from P5c (P5c made the guard fire when trusted; this makes it run unprompted): (a) **autonomy** — the interactive "trust these hooks?" prompt would freeze an unattended run; (b) **safety guarantee** — that prompt offers the agent "continue without trusting (hooks won't run)", so an agent could decline its own guards.
  - **DESIGN DECISION (Justin delegated "choose what's best for Instar", 2026-05-24 ~19:10 PDT): per-agent launch flag, not system-managed hooks.** Earlier I'd assumed managed hooks (system / requirements.toml / managed_dir) were required. Re-testing the host's real codex **0.133** corrected that: `--dangerously-bypass-hook-trust` **(i)** launches straight to ready with NO trust prompt (`⚠ Enabled hooks may run without review for this invocation`), and **(ii)** with NO trust ever granted, the guard FIRES — codex reported "the safety guard blocked it because it contains the catastrophic pattern rm -rf /" and `GUARD-FIRED` logged. (My earlier P6a note claiming the flag "still pops the prompt" conflated two launches — disproven by the clean re-test.)
  - **Why the launch flag beats system-managed hooks for Instar:** it's a flag instar's `codexCliBuilder`/headless builder fully OWN → (1) per-agent / per-launch scoping, consistent with the per-project `.codex/` decision (P1); (2) touches NOTHING system-wide — does not modify system files (a hard safety constraint) and does not intercept the operator's PERSONAL codex sessions (their manual launches lack the flag → they still get the normal trust prompt, which is correct for personal use); (3) unbypassable in the way that matters — the agent cannot strip a flag from a launch it does not construct (instar spawns it); residual hooks.json tampering is covered by always-overwrite-on-update + the file-edit gates. System-managed hooks would give absolute unbypassability but at the cost of (1)–(2), which is the wrong trade for instar.
  - **Implementation (shipped):** `src/core/codexCapabilities.ts` — memoized `codexSupportsHookTrustBypass(binaryPath)` probes `codex --help` once per binary (fails closed). `frameworkSessionLaunch.ts` — both the interactive (`codexCliBuilder`) and headless (`codexCliHeadlessBuilder`) builders append `--dangerously-bypass-hook-trust` when the probe passes. Capability-gated so codex <0.133 (which lacks the flag and would reject it, failing the whole launch) simply omits it and degrades to the safe-by-blocking trust-prompt path. NOT an agent-installed file → ships via npm update + server restart, no migration parity needed.
  - **Tests:** `tests/unit/codexCapabilities.test.ts` (5: supported/unsupported help, fail-closed missing+empty path, memoization) + `tests/unit/frameworkSessionLaunch.test.ts` (+4: interactive & headless add the flag when supported / omit when not, prompt stays last positional). Live-proven end-to-end on real codex 0.133.
- [~] **P6 — Awareness + release.** AGENTS.md briefing (Agent Awareness) + Codex-hook-disable canary + /crossreview (PermissionRequest autonomy-safety) + NEXT.md + full suite green + PR→release (v1.2.57). HARD CONSTRAINT: P5b + crossreview before merge/publish.
  ...
  - **REMAINING:**
    ...
    - `/crossreview` of the PermissionRequest autonomy-safety logic — **user-triggered** (Echo cannot launch it). <!-- tracked: codex-enforcement-hook-layer -->
    - Final consolidated release once the above land.
  - **🛑 PostCompact compaction-recovery — NOT a valid PostCompact wiring (verified 2026-05-25).** A WIP wired `compaction-recovery.sh` to Codex's `PostCompact` event for identity-survives-compaction parity. Verified against the codex 0.133 binary schema: `post-compact.command.output` exposes only `continue/stopReason/suppressOutput/systemMessage` — **no `additionalContext`**, the only output field that re-injects context into the model. Only `SessionStart`/`UserPromptSubmit` carry `additionalContext`, and Codex `SessionStart` triggers are `startup/resume/clear` (no `compact`). So wiring compaction-recovery to PostCompact installs a hook that cannot re-inject identity — dead on arrival. Codex compaction-recovery parity needs a different mechanism (candidate: ride the next `UserPromptSubmit`'s `additionalContext`, or investigate whether `systemMessage` surfaces to the model post-compaction) — its own design + spec item, not part of this layer's ship. <!-- tracked: codex-enforcement-hook-layer -->
