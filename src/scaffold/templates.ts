...
- **Reliable transfer (phrasing-independent):** \`curl -X POST -H "Authorization: Bearer $AUTH" http://localhost:${port}/pool/transfer -H 'Content-Type: application/json' -d '{"topic":N,"to":"<nickname|machineId>"}'\` → runs the same validated planner as "move this to <nickname>" but deterministically (no NL recognition). 404 unknown machine · 409 rate-limited · 409 \`needsConfirmation\` for an offline target OR an in-flight autonomous run. To confirm, re-send the same request with \`"confirm":true\` AND the returned \`confirmationChallenge\`; changed conditions return a fresh challenge instead of consuming stale consent. This is the lever to call directly when a natural-language move didn't catch.
- **Moving a topic with an autonomous run in flight (consent gate):** a transfer answers 409 \`needsConfirmation\` when the topic has a LIVE autonomous run on its current owner — moving suspends real work, so it always asks first. A challenge-bound confirmed move suspends the run at its next turn boundary (the state file survives with \`moved_to\` markers and rides the working-set carrier to the new machine — never deleted, never shipped mid-write); the response reports \`autonomousRunSuspended\`.
...
- Transition status (the ONLY way it changes — runs the gate): \`curl -X POST -H "Authorization: Bearer $AUTH" http://localhost:${port}/apprenticeship/instances/:id/transition -H 'Content-Type: application/json' -d '{"to":"active"}'\` (refused with a reason + 409 on a failed gate or illegal transition; \`complete\` is terminal after active work; \`abandoned\` is the retained terminal disposal path for a mis-created \`pending\` instance).
- Preview a gate without mutating: \`POST /apprenticeship/instances/:id/can-start\` · \`.../can-complete\`.
- Record a manual cycle: \`POST /apprenticeship/cycles\` with the id of an existing \`active\` registry instance plus positive \`cycleNumber\`, \`task\`, \`menteeOutput\`, optional \`mentorFlagged\` / \`overseerDifferential\` / \`coaching\` / \`infraItems\`, \`kind\` (\`mentor-mentee-differential\`, \`overseer-apprentice-devreview\`, \`overseer-mentee-direct\`), and \`channel\` (\`telegram-playwright\`, \`threadline-backup\`, \`direct-shortcut\`, \`unknown\`). Unknown, pending, blocked, complete, and abandoned instance references are refused. A \`telegram-playwright\` cycle additionally REQUIRES a \`transcriptAudit\` block — \`{ topicIds, window: {start,end}, summary, findingDedupKeys, generatedAt, ledger: 'local'|'remote'|'dry-run'|'failed' }\` — built from \`instar dev:post-drive-transcript-audit\` run over the drive window (use \`--history-base-url\` when the transcript lives on the mentee's server; \`ledger:'local'\` claims are cross-checked against the real framework ledger). Existing legacy dangling rows are never rewritten: inspect them with \`GET /apprenticeship/cycles/integrity\`. Use this when the overseer or manual loop found a differential outside the automated mentor tick.
...
- Cross-agent coverage: that read merges a bounded census of cycle evidence from running, registered agents on the host. Always inspect \`aggregation.complete\`, \`aggregation.omittedPeerCount\`, \`aggregation.conflictingCycleIds\`, and \`aggregation.peerSources\`: an unreachable, truncated, omitted, or UUID-conflicting source makes completeness false, so partial truth is never presented as complete role truth.
...
- **Rung 0 — Nothing:** resolve it entirely within your own permissions/accounts (own vault → identities you yourself provisioned (your owned-identities registry) → org Bitwarden → cloud accounts you're authed on (Vercel/Cloudflare/GitHub/launchd) → MCP tools → browser sessions → a resource you already control). Exhaust these FIRST. An "operator-only" verdict about infrastructure YOU built is suspect by construction — your own records are the first place to look.
- **Register what you create** (PROACTIVE — this is the trigger): the moment you provision a test/service identity (a test user, a workspace owner, a service account), register it in \`.instar/owned-identities.json\` — an array of \`{identity, service, roles, scopeTags, credentialRef}\` entries where \`credentialRef\` is a POINTER to where the credential lives, never a secret value. Register \`scopeTags\` in the SAME canonical \`service:scope\` form blocker targets use (an opaque id like \`slack:T0BA1DR0U3D\`, never a display name). The self-unblock checklist consults this registry structurally; an unregistered identity is invisible to your future exhaustion checks, and a resolving-but-stale entry blocks true-blocker settles until you prune it.
...
${ULTRACODE_SPAWN_CLAUDEMD_SECTION(port)}
...
${BENCHMARK_DIVERGENCE_CLAUDEMD_SECTION(port)}
...
${APPRENTICESHIP_STALL_GATE_CLAUDEMD_SECTION(port)}
