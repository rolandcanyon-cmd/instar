...
import type { ThreadlineClient, ReceivedMessage } from '../client/ThreadlineClient.js';
...
export class ThreadlineRESTServer {
  private readonly client: ThreadlineClient;
  private readonly config: Required<RESTServerConfig>;
  ...
  constructor(client: ThreadlineClient, config?: Partial<RESTServerConfig>) {
    ...
    this.client.on('message', (msg: ReceivedMessage) => {
  ...
  private async handleDiscover(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    const body = await this.readBody(req);
    let filter: { capability?: string; framework?: string; name?: string } | undefined;
  ...
  private async handleSend(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    ...
    let params: { recipientId?: string; message?: string; threadId?: string };
    try {
  ...
  private handleListThreads(res: http.ServerResponse): void {
  ...
  private handleGetThread(res: http.ServerResponse, threadId: string): void {
