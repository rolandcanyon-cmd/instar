 ...
 */
export function edPrivateToX25519(edPrivateKey: Buffer): Buffer {
 ...
 */
export function deriveX25519PublicKey(edPrivateKey: Buffer): Buffer {
 ...
 */
export function computeFingerprint(publicKey: Buffer): AgentFingerprint {
 ...
 */
function secureEcdh(privateKey: Buffer, publicKey: Buffer): Buffer {
 ...
 */
function deriveKey(ikm: Buffer, salt: Buffer): Buffer {
 ...
 */
function canonicalizeEnvelope(envelope: Omit<MessageEnvelope, 'signature'> & { signature?: string }): string {
  const sorted: Record<string, unknown> = {};
  for (const key of Object.keys(envelope).sort()) {
    if (key === 'signature') continue;
    sorted[key] = (envelope as Record<string, unknown>)[key];
  }
...
export class MessageEncryptor {
  ...
  private readonly x25519Private: Buffer;
  ...
  decrypt(
    ...
    senderEdPubKey: Buffer,
    senderX25519PubKey: Buffer,
  ): PlaintextMessage {
    // 1. Verify signature
