...
interface ScanState {
  /** High-water mark: last scanned message ID per topic */
  topicHighWaterMark: Record<number, number>;
...
export class CommitmentSentinel {
  private config: CommitmentSentinelConfig;
  ...
  async scan(): Promise<number> {
    ...
    try {
      ...
      const byTopic = new Map<number, TelegramMessage[]>();
   ...
   */
  private readNewMessages(): TelegramMessage[] {
    ...
    try {
      ...
      for (let i = lines.length - 1; i >= 0 && messages.length < maxPerScan * 10; i--) {
        try {
          const msg = JSON.parse(lines[i]) as TelegramMessage;
          if (!msg.messageId || !msg.topicId || !msg.text) continue;

          // Skip if we've already scanned past this message
          const hwm = this.scanState.topicHighWaterMark[msg.topicId] ?? 0;
          if (msg.messageId <= hwm) continue;
   ...
   */
  private extractConversationPairs(messages: TelegramMessage[]): Array<{ user: string; agent: string }> {
  ...
  private async detectCommitments(
    pairs: Array<{ user: string; agent: string }>,
    topicId: number,
  ): Promise<DetectedCommitment[]> {
    if (pairs.length === 0) return [];

    const conversationText = pairs.map((p, i) =>
      `--- Exchange ${i + 1} ---\nUser: ${p.user}\nAgent: ${p.agent}`
    ).join('\n\n');
...
IMPORTANT: Only return genuine commitments where the agent explicitly agreed. Do not flag questions, status updates, or informational responses. Return ONLY the JSON array, nothing else.`;
  ...
  private loadScanState(): ScanState {
    try {
