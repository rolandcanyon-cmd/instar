...
const DEFAULT_THRESHOLDS: HomeostasisThresholds = {
  commits: 3,
  minutes: 20,
};
...
export class HomeostasisMonitor {
  ...
  updateThresholds(thresholds: Partial<HomeostasisThresholds>): void {
    if (thresholds.commits !== undefined) this.data.thresholds.commits = thresholds.commits;
    if (thresholds.minutes !== undefined) this.data.thresholds.minutes = thresholds.minutes;
    this.save();
  }
  ...
  private minutesSincePause(): number {
  ...
  private load(): HomeostasisData {
    if (fs.existsSync(this.file)) {
      try {
        const raw = fs.readFileSync(this.file, 'utf-8');
        const parsed = JSON.parse(raw) as HomeostasisData;
        return {
