/**
 * Conformance suite for Interrupt primitive.
 *
 ...
 */
import type { Interrupt } from '../../primitives/control/interrupt.js';
...
export function runInterruptConformance(
  factory: ConformanceFactory<Interrupt>,
  _ctx: ConformanceContext,
): void {
  const expect = getAssertions();
  void (async () => {
    const impl = await factory();
    expect.hasCapability(impl as { capability: string }, CapabilityFlag.Interrupt);
  expect.hasMethod(impl as object, 'interrupt');
  expect.hasMethod(impl as object, 'isNonDestructive');
  })();
}
