/**
 * Conformance suite for HardKill primitive.
 *
...
export function runHardKillConformance(
  factory: ConformanceFactory<HardKill>,
  _ctx: ConformanceContext,
): void {
  const expect = getAssertions();
  void (async () => {
    const impl = await factory();
    expect.hasCapability(impl as { capability: string }, CapabilityFlag.HardKill);
