/**
 * Conformance suite for InputInjection primitive.
 *
 ...
 */
import type { InputInjection } from '../../primitives/control/inputInjection.js';
import type { ConformanceFactory, ConformanceContext } from '../runner.js';
import { CapabilityFlag } from '../../capabilities.js';
import { getAssertions } from '../runner.js';

export function runInputInjectionConformance(
  factory: ConformanceFactory<InputInjection>,
  _ctx: ConformanceContext,
): void {
  const expect = getAssertions();
  void (async () => {
    const impl = await factory();
    expect.hasCapability(impl as { capability: string }, CapabilityFlag.InputInjection);
  expect.hasMethod(impl as object, 'send');
  expect.hasMethod(impl as object, 'sendKey');
  })();
}
