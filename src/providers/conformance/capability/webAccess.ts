/**
 * Conformance suite for WebAccess primitive.
 *
 ...
 */
import type { WebAccess } from '../../primitives/capability/webAccess.js';
import type { ConformanceFactory, ConformanceContext } from '../runner.js';
import { CapabilityFlag } from '../../capabilities.js';
import { getAssertions } from '../runner.js';

export function runWebAccessConformance(
  factory: ConformanceFactory<WebAccess>,
  _ctx: ConformanceContext,
): void {
  const expect = getAssertions();
  void (async () => {
    const impl = await factory();
    expect.hasCapability(impl as { capability: string }, CapabilityFlag.WebAccess);
  expect.hasMethod(impl as object, 'buildSpec');
  expect.hasMethod(impl as object, 'supportsSearch');
  })();
}
