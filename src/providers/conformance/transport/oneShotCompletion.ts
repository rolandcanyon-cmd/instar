/**
 * Conformance suite for OneShotCompletion primitive.
 *
...
export function runOneShotCompletionConformance(
  factory: ConformanceFactory<OneShotCompletion>,
  _ctx: ConformanceContext,
): void {
  const expect = getAssertions();
  void (async () => {
    const impl = await factory();
    expect.hasCapability(impl as { capability: string }, CapabilityFlag.OneShotCompletion);
