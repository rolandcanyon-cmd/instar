...
export type HighRiskAction =
  ...
  | 'git-push'
  | 'external-api'
  | 'file-modify-outside-project'
  | 'production-change';

export class CoherenceGate {
  private config: CoherenceGateConfig;
   ...
   */
  private saveTopicBindings(): void {
    const bindingsPath = path.join(this.config.stateDir, 'topic-project-bindings.json');
  ...
  private checkWorkingDirectory(): CoherenceCheck {
    let cwd: string;
    try {
      cwd = process.cwd();
    } catch {
  ...
  private checkGitRemote(): CoherenceCheck {
  ...
  private checkTopicProjectAlignment(topicId: number): CoherenceCheck {
  ...
  private checkDeploymentTarget(targetUrl: string, topicId?: number): CoherenceCheck {
    ...
    if (topicId) {
      ...
      if (binding?.deploymentTargets && binding.deploymentTargets.length > 0) {
        const allowed = binding.deploymentTargets.some(t =>
          targetUrl.includes(t) || t.includes(targetUrl),
        );
  ...
  private checkPathScope(targetPath: string): CoherenceCheck {
  ...
  private checkAgentIdentity(): CoherenceCheck {
