...
// ---------------------------------------------------------------------------
// Extended context — passed per-review, NOT stored on instance (stateless contract)
// ---------------------------------------------------------------------------
...
export interface EscalationReviewResult extends ReviewResult {
  ...
  researchContext?: { blockerDescription: string; capabilities?: CapabilityRegistry };
}
...
const AUTONOMY_GUIDANCE: Record<AutonomyLevel, string> = {
  autonomous:
    'This agent is configured as AUTONOMOUS. The bar for allowing escalation should be HIGH. ' +
    'Only allow if the agent genuinely lacks the capability.',
  collaborative:
    'This agent is COLLABORATIVE. Apply balanced judgment — block clear unnecessary escalations, ' +
    'allow genuine ones.',
  supervised:
    'This agent is SUPERVISED. Apply moderate judgment — only block obviously unnecessary escalations.',
  cautious:
    'This agent is CAUTIOUS. Allow most escalations — only block if the resolution is trivially ' +
...
export class EscalationResolutionReviewer extends CoherenceReviewer {
   ...
   */
  async review(context: EscalationReviewContext): Promise<EscalationReviewResult> {
   ...
   */
  private parseEscalationResponse(raw: string): {
    pass: boolean; severity: string; issue: string; suggestion: string; confidence: number;
  } {
    ...
    try {
      ...
      return {
        ...
        severity: (parsed['severity'] as string) ?? 'warn',
        issue: (parsed['issue'] as string) ?? '',
  ...
  protected buildPrompt(context: EscalationReviewContext): string {
    ...
    const autonomyLevel: AutonomyLevel = context.autonomyLevel ?? 'collaborative';
...
${this.wrapMessage(context.message, boundary)}`;
  ...
  sanitizeRegistry(registry: CapabilityRegistry): string {
    ...
    if (registry.authentication) {
      sanitized.authentication = Object.fromEntries(
    ...
    if (registry.tools) {
      ...
      sanitized.tools = Object.fromEntries(
        Object.entries(registry.tools).map(([k, v]) => [k, {
          tool: v.tool,
    ...
    if (registry.accountsOwned) {
      ...
      sanitized.accountsOwned = Object.fromEntries(
   ...
   */
  private matchKnownBlocker(context: EscalationReviewContext): CommonBlocker | null {
    if (!context.jobBlockers) return null;
    ...
    for (const [_key, blocker] of Object.entries(context.jobBlockers)) {
      ...
      if (blocker.expiresAt && new Date(blocker.expiresAt) < new Date()) continue;
      // Skip pending blockers (not yet confirmed)
      if (blocker.status === 'pending') continue;
