...
import type { FeatureDefinition, ConsentTier, DataImplication } from './FeatureRegistry.js';
 ...
 */
export function awarenessMessage(definition: FeatureDefinition): SurfacingMessage {
 ...
 */
export function generateSurfacingMessage(
  definition: FeatureDefinition,
  level: SurfaceLevel,
  context?: SurfacingContext,
): SurfacingMessage {
 ...
 */
export function validateSurfacingMessage(message: SurfacingMessage): string[] {
 ...
 */
function formatDataImplications(implications: DataImplication[]): string {
  if (implications.length === 0) return 'no data collected';
 ...
 */
export function recommendedSurfaceLevel(tier: ConsentTier): SurfaceLevel {
  switch (tier) {
