...
function computeScore(
  ...
  if (isEvergreen) return normalizedScore;
...
export class MemoryIndex {
  ...
  sync(): { added: number; updated: number; removed: number } {
    ...
    for (const { filePath, relativePath, source } of sourceFiles) {
      ...
      const existing = this.db.prepare('SELECT hash FROM tracked_files WHERE path = ?').get(relativePath) as { hash: string } | undefined;

      if (existing && existing.hash === hash) continue; // Unchanged
    ...
    const trackedPaths = (this.db.prepare('SELECT path FROM tracked_files').all() as { path: string }[]).map(r => r.path);
   ...
   */
  search(query: string, options?: { limit?: number; source?: string }): MemorySearchResult[] {
    if (!this.db) throw new Error('Database not open. Call open() first.');
  ...
  stats(): MemoryIndexStats {
    ...
    const totalFiles = (this.db.prepare('SELECT COUNT(*) as cnt FROM tracked_files').get() as { cnt: number }).cnt;
    const totalChunks = (this.db.prepare('SELECT COUNT(*) as cnt FROM chunks').get() as { cnt: number }).cnt;
    const lastIndexedAt = (this.db.prepare("SELECT value FROM meta WHERE key = 'last_indexed_at'").get() as { value: string } | undefined)?.value || '';
  ...
  private indexFile(
    ...
    const insertMany = this.db.transaction((chunks: Array<{ text: string; offset: number; length: number; tokenCount: number }>) => {
   ...
   */
  private collectSourceFiles(): Array<{ filePath: string; relativePath: string; source: MemorySource }> {
    const files: Array<{ filePath: string; relativePath: string; source: MemorySource }> = [];
