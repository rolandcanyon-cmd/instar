...
`, { mode: 0o755 });
  ...
  // PreToolUse hook. When a tool looks like an external service operation (mcp__*),
  // classifies the operation and calls the gate API for risk evaluation.
  // Can block dangerous operations or inject advisory context for high-risk ones.
  fs.writeFileSync(path.join(hooksDir, 'external-operation-gate.js'), `#!/usr/bin/env node
