...
/**
 * Generate and install a boot wrapper script that resolves the shadow install
 ...
 *
 * Stored at {projectDir}/.instar/instar-boot.sh — stable path, updated on setup.
 */
function installBootWrapper(projectDir: string): string {
...
function installMacOSLaunchAgent(projectName: string, projectDir: string, hasTelegram: boolean): boolean {
  ...
  // Install a boot wrapper that resolves shadow install at startup time.
  ...
  const wrapperPath = installBootWrapper(projectDir);
  ...
  // The wrapper is a bash script that resolves node + CLI internally
  const programArgs = ['/bin/bash', wrapperPath, ...args];
